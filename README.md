...
`https://v4.gh-proxy.org/`。版本检测始终按 `checkUrl` 原始地址请求，不走代理；
代理只会在最终 ZIP 下载地址为 `https://github.com/...` 时临时拼成
`https://v4.gh-proxy.org/https://github.com/...`。配置文件仍保存原始 `checkUrl`
与单独的 `downloadProxyPrefix`。
...
`POST /api/u8/sales-order/save`。官方 C# 示例目录中的通用能力也迁移为同一风格的
业务化路径，例如 `POST /api/u8/ap-apply-pay/cancel-sign` 内部调用
`U8API/APApplyPay/CancelSign`。`POST /api/u8/official/{官方地址}` 仅作为
legacy/internal 兼容入口保留给旧调用和内部排查，新对接不要使用
`/official/U8API/...` 风格路径。
