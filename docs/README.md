...
对外推荐路径统一为 `POST /api/u8/<business-resource>/<action>`，例如 `POST /api/u8/sales-order/save`、`POST /api/u8/material-out/add`、`POST /api/u8/bom/add`。官方 C# 示例目录中的通用能力也按同一规则迁移为业务化 alias，例如 `POST /api/u8/ap-apply-pay/cancel-sign` 内部调用 `U8API/APApplyPay/CancelSign`。旧 `POST /api/u8/official/{官方地址}` 仅保留为 legacy/internal 兼容入口，用于老调用或内部排查；新对接不要使用 `/official/U8API/...` 作为业务路径。
